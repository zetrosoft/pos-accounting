...
if (! function_exists('no_ser')){
	function no_ser(){
		...
		$findme ='Product ID';// 'Physical';
		$pmac = strpos($mycom, $findme); 
		$mac=md5(substr($mycom,($pmac+27),23)); 
		return $mac;
...
// add function by zetrosoft
if ( ! function_exists('sepasi'))
{
	function sepasi($num = 1)
	{
		return str_repeat(" ", $num);
	}
}

if ( ! function_exists('tgl_local'))
{
		function tgl_local($tgl=''){
			if($tgl==''){
			return date('d/m/Y');
			}else{
				$tgl=str_replace("-","",$tgl);
...
/* add function by Iswan Putera S.Kom 
   create short tag table 
*/
if (! function_exists('tabel')){
...
if (! function_exists('tr')){
	function tr($class='xx', $id=''){
		return "<tr class='$class' id='$id'>\n";
	}
	function _tr(){
		return "</tr>\n";
	}
