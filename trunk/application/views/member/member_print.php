<?php
		  ...
		  $a->SetFont('Arial','',9);
		  //$rec = $temp_rec->result();
		  $n=0;$debet=0;$hgb=0;$kredit=0;$saldo=0;
		  foreach($temp_rec->result() as $r)
		  {
			$n++;
			$a->Row(array($n, tglfromSql($r->Tanggal),
				$r->Nomor,$r->Keterangan.' '.$ID_Calc,
				number_format($r->Debet,2),number_format($r->Kredit,2)));
			//sub tlot
			$kredit =($kredit+$r->Kredit);
			$debet =($debet+$r->Debet);
			$saldo=($ID_Calc==1)?($debet-$kredit):($kredit-$debet);
		  }
		  $a->SetFont('Arial','B',10);
		  $a->SetFillColor(225,225,225);
		  $a->Cell(124,8,"TOTAL",1,0,'R',true);
		  $a->Cell(28,8,number_format($debet,2),1,0,'R',true);
		  $a->Cell(28,8,number_format($kredit,2),1,1,'R',true);
