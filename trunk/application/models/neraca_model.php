...
class Neraca_model extends CI_Model {
	...
	function get_nc_lajure_tg($st,$sa,$dept,$simpanan)
	{
			$sql="call s_neraca_lanjur_by_tanggal(".tgltoSql($st).",".tgltoSql($sa).",".$dept.",".$simpanan.")";
			$data=$this->db->query($sql);
				return $data->result();
	}
	function get_nc_lajure($periode,$dept,$simpanan){
/*		$this->build_data($periode);
		$sql="select t.ID_P,t.ID_Agt,t.ID_Klas,t.ID_SubKlas,t.ID_Unit,t.ID_Dept,t.ID_Simpanan,t.ID_Calc,sum(kredit) as Kredit,sum(debet) as Debet
			...
			// 
*/				
			$sql="call s_neraca_lajur_by_tahun(".$periode.",".$dept.",".$simpanan.")";
			$data=$this->db->query($sql);
				return $data->result();
