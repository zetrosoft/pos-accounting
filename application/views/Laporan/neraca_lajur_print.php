<?php
		...
		foreach($temp_rec as $r){
			...
			$a->Row(array($n,$kode,rdb('mst_anggota','Nama','Nama',"where ID='".$r->ID_Agt."'")." - ".$simp,
						number_format($saldoawale,0),number_format($r->Debet,0),number_format($r->Kredit,0),
						number_format($saldoawale+$saldo,0)
						)
						);
			$s_pokok=($s_pokok+$saldoawale);
			$s_wajib=($s_wajib+$r->Debet);
			$s_kusus=($s_kusus+$r->Kredit);	
			$s_akhir=($s_akhir+$saldoawale+$saldo);
		}
		  $a->SetFont('Arial','B',10);
