...
$(document).ready(function(e) {
	...
	$('#SubTable').hide()
	$('#SubSHU').hide()
	$('#ListTable').fixedHeader({width:(screen.width-250),height:170/*(screen.height-320)*/})
	$('#HeadSHU').fixedHeader({width:(screen.width-250),height:240/*(screen.height-320)*/})
	show_detail('1');
	$('#aktif').val('8')
	$('#calc').val('2');
...
function show_detail(id){
	...
	$('#aktif').val(idd[0]);
	$('#calc').val(idd[1]);
	$('#ListTable_body_container #ListTable tbody tr#c-'+idd[0]).addClass('list_genap');
	$('#ListTable_body_container #ListTable tbody tr:not(#c-'+idd[0]+')').removeClass('list_genap');
	$.post('get_subneraca',{'ID':idd[0]},
		function(result){
			$('#SubTable tbody').html(result);
			$('#SubTable').fixedHeader({width:(screen.width-400),height:240})
			$('#SubTable').show()
		})
}
